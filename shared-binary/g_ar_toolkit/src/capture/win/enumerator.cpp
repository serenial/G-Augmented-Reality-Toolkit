...
Enumerator::Enumerator() : m_last_state(states::STARTING),
...
void Enumerator::update_last_device_enumeration()
{
    ...
    for (UINT i = 0; i < device_count; i++)
    {
        ...
        // winrt::to_string() doesn't use the same code page as labview so do this conversion manually

        // get the length of the converted string
        auto required_ascii_buffer_length = WideCharToMultiByte(CP_ACP, 0, &property[0], property_length , nullptr, 0, NULL, NULL);
        // resize the buffer to write to
        device_info.device_name.resize(required_ascii_buffer_length);
        // convert into the buffer
        WideCharToMultiByte(CP_ACP, 0, &property[0], property_length , device_info.device_name.data(), required_ascii_buffer_length, NULL, NULL);

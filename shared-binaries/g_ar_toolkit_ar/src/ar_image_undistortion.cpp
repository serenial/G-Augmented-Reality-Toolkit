...
namespace
{
    ...
    {
    public:
        ImageUndistorter(cv::Mat camera_mat, cv::Mat distortion_coeffs, cv::Size input_size, double alpha);
        ImageUndistorter() = delete;
        void undistort(const cv::Mat &src, cv::Mat &dst, bool use_fixed_point_maps, bool crop_to_valid, int border_mode, cv::Scalar background) const;
...
extern "C"
{
    G_AR_TOOLKIT_AR_EXPORT LV_MgErr_t g_ar_tk_ar_image_undistortion_map_create(
        ...
        LV_ImageSizePtr_t roi_size_ptr)
    {
        try
        {
            EDVRManagedObject<ImageUndistorter> image_undistorter(edvr_ref_ptr, new ImageUndistorter(camera_matrix_ptr->as_cv_mat(), camera_distortion_ptr->as_cv_mat(), *size_ptr, alpha));

            image_undistorter->copy_new_camera_mat_to(camera_matrix_ptr->as_cv_mat());

            *roi_top_left_ptr = image_undistorter->get_valid_pixel_rect().tl();
            *roi_size_ptr = image_undistorter->get_valid_pixel_rect().size();
        }
        catch (...)
        {
    ...
    G_AR_TOOLKIT_AR_EXPORT LV_MgErr_t g_ar_tk_ar_image_undistortion_map_undistort_image(
        LV_ErrorClusterPtr_t error_cluster_ptr,
...
ImageUndistorter::ImageUndistorter(cv::Mat camera_mat, cv::Mat distortion_coeffs, cv::Size input_size, double alpha)
...
void ImageUndistorter::copy_new_camera_mat_to(cv::Mat dst) const
{
    m_new_camera_mat.copyTo(dst);
}

cv::Rect ImageUndistorter::get_valid_pixel_rect() const
{
    return m_valid_pixel_rect;
}

void ImageUndistorter::undistort(const cv::Mat &src, cv::Mat &dst, bool use_fixed_point_maps, bool crop_to_valid, int border_mode, cv::Scalar background) const
{
    ...
    if (crop_to_valid)
    {
        // use internal image_undistorter buffer to accept result and then copy roi into dst
        ...
        cv::remap(src, buffer, m1, m2, cv::INTER_LINEAR, border_mode, background);
    ...
    else
    {
        ...
        cv::remap(src, dst, m1, m2, cv::INTER_LINEAR, border_mode, background);
    }
}

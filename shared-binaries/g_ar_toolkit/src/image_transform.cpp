...
extern "C"
{
    ...
    G_AR_TOOLKIT_EXPORT LV_MgErr_t g_ar_tk_image_get_perspective_transform(
        LV_ErrorClusterPtr_t error_cluster_ptr,
        LV_1DArrayHandle_t<LV_ImagePointFloat_t> A,
        LV_1DArrayHandle_t<LV_ImagePointFloat_t> B,
        LV_3x3MatrixPtr_t warp_mat_ptr)
    {
        try
        {
            *warp_mat_ptr = cv::getPerspectiveTransform(A.to_vector<cv::Point2f>(), B.to_vector<cv::Point2f>());
        }
        catch (...)
        {
            error_cluster_ptr.copy_from_exception(std::current_exception(), __func__);

...
namespace
{
    ...
    public:
        ...
        void distort(const cv::Mat &src, cv::Mat &dst, bool use_fixed_point_maps) const;
...
extern "C"
{
        ...
        LV_EDVRReferencePtr_t edvr_ref_ptr)
    {
        try
        {
            volatile EDVRManagedObject<ImageDistorter> image_rectifier(edvr_ref_ptr, new ImageDistorter(camera_matrix_ptr->as_cv_mat(), camera_distortion_ptr->as_cv_mat(5), *size_ptr));
        }
        catch (...)
        {
        ...
        LV_BooleanPtr_t use_fixed)
    {
        try
        {
            ...
            image_distorter->distort(src, dst, *use_fixed);
        }
        catch (...)
        {
            error_cluster_ptr->copy_from_exception(std::current_exception(), __func__);
...
void ImageDistorter::distort(const cv::Mat &src, cv::Mat &dst, bool use_fixed_point_maps) const
{

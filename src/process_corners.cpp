...
using LV_ChessboardPoints_t = struct
{
...
extern "C"
{
    G_AR_TOOLKIT_EXPORT LV_MgErr_t g_ar_tk_find_chessboard_corners(
        ...
        LV_ChessboardPoints_t* points_ptr,
        LV_Handle_t<LV_Array_t<1,uint8_t>> flags_array_handle,
        LV_CornerArrayHldPtr_t corners_handle)
    {
        try
        {
            ...
            for(size_t i=0; i<array_handle_array_length(flags_array_handle); i++){
                flags += find_chessboard_corners_enum_to_flag((*flags_array_handle)->data[i]);
            }
            ...
            copy_with_allocation_to_1d_lv_array_handle_ptr<std::vector<cv::Point2f>, LV_Corner_t>(corners, corners_handle, [](LV_Corner_t *out,bool _, cv::Point2f in){
                out->x = in.x;
                out->y = in.y;
            });
        }
